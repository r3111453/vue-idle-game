...
export default {
  ...
  data() {
    return {
      ...
      // 记录当前正在强化的装备槽位（用于强化后同步）
      strengthenTargetSlot: null
    };
  },
  ...
  watch: {
    ...
    // 监听强化面板关闭，同步强化后的装备到身上
    strengthenEquipmentPanelOpened(newVal, oldVal) {
      if (oldVal === true && newVal === false && this.strengthenTargetSlot) {
        // 强化面板关闭，且存在待同步的槽位
        const strengthenedEquip = this.$store.state.needStrengthenEquipment;
        if (strengthenedEquip && strengthenedEquip.type) {
          // 更新对应槽位的装备
          switch (this.strengthenTargetSlot) {
            case 'weapon':
              this.$store.commit('set_player_weapon', this.$deepCopy(strengthenedEquip));
              break;
            case 'armor':
              this.$store.commit('set_player_armor', this.$deepCopy(strengthenedEquip));
              break;
            case 'ring':
              this.$store.commit('set_player_ring', this.$deepCopy(strengthenedEquip));
              break;
            case 'neck':
              this.$store.commit('set_player_neck', this.$deepCopy(strengthenedEquip));
              break;
          }
          // 保存游戏
          this.saveGame(false);
        ...
        // 清除记录
        this.strengthenTargetSlot = null;
      }
    }
  },
  methods: {
    ...
    strengthenEquipmentFromContext() {
      ...
      // 记录要强化的槽位
      this.strengthenTargetSlot = this.equipmentContextMenu.slot;
      // 将当前装备深拷贝存入 store，供强化面板使用
